...
export function formatTaskState(val: number): string {
  // 1、待处理 2、待确认 3、待入账 4、成功 5、钱包处理失败 6、钱包处理中
  switch (val) {
    case 1:
      return '待处理';
    case 2:
...
export function numberToChinese(number: number): string {
  ...
  const chineseUnits: string[] = ['', '十', '百', '千', '万', '十万', '百万', '千万', '亿'];
  ...
  while (number > 0) {
    ...
    if (digit !== 0) {
      ...
      chineseStr = chineseNumbers[digit] + chineseUnits[unitIndex] + chineseStr;
    } else {
      zeroFlag = true;
    }
...
export function shuffleArray<T>(array: T[]): T[] {
  // 打乱数组顺序
  const newArray = [...array]; // 创建原数组的副本，以避免修改原数组
...
export function toCapital(n: number | string): string {
  ...
  const s: string = cNum.reduce(
    (acc, cur, index, arr) => acc + arr[window.parseInt(n.toString().charAt(Number(cur)), 10)],
    ''
  );
        ...
        ...(_value ?? _key) // 映射或过滤的key
          ...
          .reduce(
            (shines, __key: string) => ({
              ...shines,
              [__key]:
                typeof callback === 'undefined' ? item[_key] : callback(__key, item[_key], item),
            }),
            Object.create(null)
          ),
      }),
...
export function makeFilterUnique<T extends { [key: string]: any }>( // 数组去重,支持递归去重
  ...
  return Object.values(
    (buildFlat<T>(array, key, false) as T[]).reduce(
      (acc, cur) => Object.assign(acc, { [cur[filterKey]]: cur }),
      Object.create(null)
    )
  );
}
...
export function debounceFunc(fn: (...arg: any[]) => any, time = 500): ReturnType<any> {
  // 防抖
...
export function throttleFunc(fn: (...arg: any[]) => any, time = 500): ReturnType<any> {
  // 截流
...
export const onCopyCode = (val: string): void => {
  // 复制code / 链接
  try {
    copyToClipboard(val);
