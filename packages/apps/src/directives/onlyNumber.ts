...
export const onlyNumber: Directive = function (
  ...
  const digit = Number(/\./.test(stringArg) ? stringArg.split('.').slice(-1)[0].length : stringArg);
  // eslint-disable-next-line no-useless-escape
  const transformValue =
    String(value).split('.')[1]?.length > digit
      ? Number(value)
          .toFixed(digit + 1)
          .slice(0, -1)
      : value; // 防止用户复制粘贴
  const newValue = new RegExp(`^\\-?\\d*\\.?\\d{0,${digit}}$`).test(transformValue)
    ? transformValue
    : oldValue; // 处理用户正常输入

  const inputEl: HTMLInputElement =
    el.tagName === 'INPUT' ? <HTMLInputElement>el : <HTMLInputElement>el.querySelector('input');
  ...
  _updateValue(
    typeof oldValue === 'undefined'
      ? Number(value)
          .toFixed(digit + 1)
          .slice(0, -1) /* 第一次及时处理原始值的小数位数，后续再失去焦点才补全小数位数 */
      : newValue
  );
};
